...
(async () => {
  ...
  const { BlockAdmin, DistrictAdmin, StateAdmin } = getAdminModels(mongoose.connection);
  ...
  for (let s = 0; s < (data.states || []).length; s++) {
    ...
    if (!stateName) { errors.push(`Missing state name at index ${s}`); continue; }
    ...
    try {
      ...
      if (saRes.upsertedCount) { created++; }
      else { skipped++; }
    } catch (err) {
      if (err.code === 11000) { skipped++; }
      else { errors.push(`State admin error (${stateName}): ${err.message}`); }
    }
    ...
    for (let d = 0; d < districts.length; d++) {
      ...
      if (!distName) { errors.push(`Missing district name at state ${stateName}`); continue; }
      ...
      try {
        ...
        if (daRes.upsertedCount) { created++; }
        else { skipped++; }
      } catch (err) {
        if (err.code === 11000) { skipped++; }
        else { errors.push(`District admin error (${distName}, ${stateName}): ${err.message}`); }
      }
      ...
      for (let b = 0; b < blocks.length; b++) {
        ...
        if (!blockName || blockName === '-') continue;
        ...
        try {
          ...
          if (baRes.upsertedCount) { created++; }
          else { skipped++; }
        } catch (err) {
          if (err.code === 11000) { skipped++; }
          else { errors.push(`Block admin error (${blockName}, ${distName}, ${stateName}): ${err.message}`); }
        }
      }
    }
  }
