...
router.get('/:identifier/details', async(req, res) => {
    try {
        ...
        if (decodedIdentifier.includes('@')) {
            ...
            [personalDetails, businessInfo, financialInfo, declaration] = await Promise.all([
                MemberDetails.findOne({ email: decodedIdentifier }).lean(),
                MemberBusinessInfo.findOne({ email: decodedIdentifier }).lean(),
                MemberFinancialInfo.findOne({ email: decodedIdentifier }).lean(),
                MemberDeclaration.findOne({ email: decodedIdentifier }).lean()
            ...
            try {
                ...
                [personalDetails, businessInfo, financialInfo, declaration] = await Promise.all([
                    MemberDetails.findById(objectId).lean(),
                    MemberBusinessInfo.findOne({ memberId: objectId }).lean(),
                    MemberFinancialInfo.findOne({ memberId: objectId }).lean(),
                    MemberDeclaration.findOne({ memberId: objectId }).lean()
                ]); // ✅ OPTIMIZED: Parallel queries with .lean()
            } catch (err) {
                console.log('⚠️ Invalid ObjectId format, trying as string');
                // Fallback: try as string
                [personalDetails, businessInfo, financialInfo, declaration] = await Promise.all([
                    MemberDetails.findOne({ _id: decodedIdentifier }).lean(),
                    MemberBusinessInfo.findOne({ memberId: decodedIdentifier }).lean(),
                    MemberFinancialInfo.findOne({ memberId: decodedIdentifier }).lean(),
                    MemberDeclaration.findOne({ memberId: decodedIdentifier }).lean()
                ]); // ✅ OPTIMIZED: Parallel queries with .lean()
            }
        }
        ...
        const memberData = {
            _id: (personalDetails && personalDetails._id) || null,
    ...
    try {
        ...
        if (personal_and_demographic_details) {
            await MemberDetails.findOneAndUpdate(
                queryCriteria,
                personal_and_demographic_details, { new: true }
            );
        }
        ...
        if (financial_and_compliance) {
            await MemberFinancialInfo.findOneAndUpdate(
                decodedIdentifier.includes('@') ? { email: decodedIdentifier } : { memberId: queryCriteria._id },
                financial_and_compliance, { new: true }
            );
        }
