...
async function activateMembership(paymentData) {
    try {
        ...
        // ✅ OPTIMIZED: Query with indexed email: field, uses .select() .lean() maxTimeMS()
        const result = await MemberDetails.findOneAndUpdate({ email: paymentData.email }, { membershipStatus: 'active', membershipType: membershipType, membershipActivatedAt: new Date(), membershipExpiresAt: expiresAt, paymentId: paymentData.paymentId, paymentAmount: amount, lastPaymentDate: new Date() }, { new: true, lean: true, select: 'email fullName membershipStatus membershipType membershipActivatedAt membershipExpiresAt', maxTimeMS: 1000 });
