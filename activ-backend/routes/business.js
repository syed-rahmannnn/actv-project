...
router.get('/companies', async(req, res) => {
    try {
        ...
        // ✅ OPTIMIZED: Query with indexed memberId: field, uses .select() .lean() .maxTimeMS()
        const companies = await MemberBusinessInfo.find({ memberId: memberId }).select('_id organizationName businessType mobile status').lean().maxTimeMS(500);
...
router.post('/profile', async(req, res) => {
    try {
        ...
        // ✅ OPTIMIZED: Update using indexed memberId: with .select() and .lean()
        const businessInfo = await MemberBusinessInfo.findOneAndUpdate({ memberId: memberId }, {
            ...businessData,
            memberId: memberId
        }, {
            upsert: true,
            ...
            runValidators: true,
            lean: true,
            select: '_id organizationName businessType businessDescription location businessWebsite logoUrl memberId',
            maxTimeMS: 1000
        });
